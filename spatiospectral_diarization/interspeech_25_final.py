...
@ex.capture
def spatio_spectral_pipeline(json_path, dsets,setup,full_algorithm, tmp_class_th_compact, channels,experiment_dir, noctua2, noctua1, debug):
    ...
    if distributed:
        ...
        f_max_gcc = None
        avg_len_gcc = 4
        max_diff_tmp_cl = tmp_class_th_compact
        max_temp_dist_cl = 16
        min_srp_peak_rato = .5
    ...
    for dset in dsets:
        ...
        for session in dlp_mpi.split_managed(dataset, allow_single_worker=True):
            ...
            if subset == 'libricss':
                gt_activities = {}
                for onset, num_smpls, spk_id in zip(example['onset']['original_source'], example['num_samples']['original_source'], example['speaker_id']):
                    if spk_id not in gt_activities:
                        gt_activities[spk_id] = np.zeros(sigs.shape[-1], bool)
                    gt_activities[spk_id][onset:onset+num_smpls] = 1
                gt_activities = np.asarray([a for a in gt_activities.values()])
            elif subset == 'libriwasn200':
                if noctua2:
                    gt_activities = pb.array.interval.from_rttm(
                        '/scratch/hpc-prf-nt1/cord/jsons/ref_diarization_libriwasn_200.rttm'
                    )
                elif noctua1:
                    gt_activities = pb.array.interval.from_rttm(
                        '/scratch-n2/hpc-prf-nt1/cord/jsons/ref_diarization_libriwasn_200.rttm'
                    )
                else:
                    gt_activities = pb.array.interval.from_rttm(
                        '/net/vol/tgburrek/db/libriwasn/ref_diarization_libriwasn_200.rttm'
                    )
                gt_activities = gt_activities[example['example_id']]
                gt_activities_ = []
            ...
            elif subset == 'libriwasn800':
                if noctua2:
                    gt_activities = pb.array.interval.from_rttm(
                        '/scratch/hpc-prf-nt1/cord/jsons/ref_diarization_libriwasn_800.rttm'
                    )
                elif noctua1:
                    gt_activities = pb.array.interval.from_rttm(
                        '/scratch-n2/hpc-prf-nt1/cord/jsons/ref_diarization_libriwasn_200.rttm'
                    ...
                    gt_activities = pb.array.interval.from_rttm(
                        '/net/vol/tgburrek/db/libriwasn/ref_diarization_libriwasn_800.rttm'
                    )
                gt_activities = gt_activities[example['example_id']]
                gt_activities_ = []
                for act in gt_activities.values():
            ...
            # spatial_om[session_name] = {spk: pb.array.interval.ArrayInterval(act.astype(bool))
            #                                        for spk, act in enumerate(est_activities_om)}
        ...
        if dlp_mpi.IS_MASTER:
            ...
            pb.array.interval.rttm.to_rttm(diarization_targets, Path(experiment_dir) / f'diarization_targets_{dset}.rttm')
        dlp_mpi.barrier()
